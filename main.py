...
#unsafe af, move to git secrets but doesn't matter so much since it's a free read-only token
...
class Chatbot:
    ...
    def ask_question(self, user_query):
        ...
        rag_chain_model = (
                ...
                | (lambda docs: {"context": self.context,
                                 "query": user_query})  # Combine context and query
...
def main():
    ...
    model = HuggingFaceEndpoint(repo_id=llm_repo_id, temperature=0.5, max_new_tokens=2048, model_kwargs=model_kwargs)
